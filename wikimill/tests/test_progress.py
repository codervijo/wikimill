...
from wikimill.progress import Heartbeat
...
def test_a_stalled_stage_is_visible_on_the_page(tmp_path):
    ...
    with open_db(tmp_path / "w.db") as conn:
        make(conn, updated=T0, current="http://hanging.example/x")
        data = report_mod.collect(conn)
        data.generated_at = "2026-07-25T12:30:00+00:00"
...
def test_the_page_only_auto_refreshes_while_something_runs(tmp_path):
    ...
    with open_db(tmp_path / "w.db") as conn:
        make(conn, updated=T0, finished=T0, outcome="ok")
        done = report_mod.render(report_mod.collect(conn), refresh_seconds=10)
        assert "http-equiv='refresh'" not in done

        conn.execute("DELETE FROM run_progress")
        make(conn, updated=T0)
        data = report_mod.collect(conn)
        data.stages = progress_mod.snapshot(conn, T0)
